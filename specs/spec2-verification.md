...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Ninguna salida del bucle de capítulo deja estado del capítulo sin cerrar | 1 | Integration testing, una prueba por tipo de salida | `T` | Solo las salidas que el test enumera; una excepción nueva en otro sitio no está cubierta | No: la regla de integridad lo ve después | `tests/test_auditoria.py::test_hallazgo_05_…` (xfail) | pendiente |
| 2 | El worker caído deja el grafo en el último capítulo íntegro | 1 | Integration testing con una caída real del proceso | `T` | Una caída a mitad de una transacción la cubre SQLite, no este test | No | `tests/test_auditoria.py::test_hallazgo_06_…` (xfail) | pendiente |
| 3 | Ningún capítulo se genera sin las puertas 1 y 2 vigentes | 2 | Model checking de estados + `avanzar` | `A` | Solo ve el orden; se fía de que `resultado_puerta` sea veraz | No | `tests/test_auditoria.py::test_hallazgo_01_…` (xfail) | pendiente |
| 4 | Lo mismo, en ejecución | 2 | Guardarraíl en `generar_capitulo` | `D` | El mismo | No | — | pendiente |
| 5 | Resolver una parada rehace lo que la puerta rechazó | 2 | Integration testing | `T` | Comprueba que se rehace, no que la segunda versión sea mejor | **Sí** | `tests/test_auditoria.py::test_hallazgo_02_…`, `::test_hallazgo_12_…` (xfail) | pendiente |
| 6 | Dos workers no escriben a la vez | 3 | Integration testing con dos procesos | `T` | Un proceso congelado más tiempo que la gracia, que escribe en la transacción que ya tenía abierta | No: *fencing* + latido | `tests/test_auditoria.py::test_hallazgo_03_…` (xfail) | pendiente |
| 7 | Ningún elemento obligatorio se pierde; ningún recorte es silencioso | 4 | Property-based testing | `T` | Lo marcado opcional puede ser lo que el capítulo necesitaba: la relevancia es heurística | **Sí** | `tests/test_auditoria.py::test_hallazgo_04_…`, `::test_hallazgo_10_…` (xfail) | pendiente |
