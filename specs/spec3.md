...
- **Los hechos vigentes de esos personajes, de los objetos de las escenas del capítulo y de las facciones del reparto.** Son opcionales, como los de mundo y novela, y se recortan del más antiguo al más reciente.
- **Dos reglas en la skill.** Una cuenta de personas, horas, plazos o raciones sale de los hechos establecidos y cuadra con ellos. Y a quien la escaleta no puso solo se le trae si está entre los que ya han salido, como dicen su ficha y sus hechos.

> **Decisión sin entrevistar.** La regla de las cuentas es un prompt, y no una comprobación, porque la puerta 3 no suma ni resta entre hechos de atributos distintos: es el falso negativo C12 del banco (fila 50). Prevenir en el redactor es lo más barato. La detección, con un juez que revise las cifras contra el canon, va con el bloque 6. Se descartó hacer obligatorias las fichas de los que ya salieron: con un elenco grande, desplazarían del paquete el canon del reparto, que es el que el capítulo usa seguro.

### Quién está, para la muerte y la ubicuidad

**RF3-PAS-11 — La muerte y la ubicuidad leen la prosa, no la escaleta.** Las dos comprobaciones de `presencia_imposible` (un muerto que reaparece y alguien en dos lugares a la vez) preguntan si alguien está en una escena. Hasta aquí lo contestaba el reparto, que la escaleta planifica antes de la prosa. En la escena 4.4 de la novela real, el reparto conservaba a tres personajes que ya se habían ido, y la prosa no los traía. En el banco de contraejemplos es el falso positivo L08. Desde aquí, **estar**, para estas dos comprobaciones, es:
- la presencia que constata el extractor (RF3-PAS-09);
- el POV;
- un uso de conocimiento registrado en la escena;
- un estado del personaje registrado en la escena, salvo el que repite su muerte: encontrar el cadáver no trae al muerto;
- el reparto, **solo en una escena sin ninguna presencia constatada**. Es lo que dejaba el extractor antes de RF3-PAS-09, y así una base vieja no pierde la detección.

La skill del extractor añade a la regla de las presencias que un cadáver no es una presencia: a un personaje muerto solo lo registra si la escena lo muestra hablando o actuando.

> **Decisión sin entrevistar.** El objetivo es la novela con menos fallos, y aquí se cambia una parada sin motivo (L08) por un punto ciego nuevo: si la prosa trae a un muerto y el extractor no lo registra, ya no para (C16, con el mismo grafo que L08). Se acepta por tres razones. El extractor registra las presencias con una precisión medida de 17 de 17 en el capítulo 4 de la novela real, sin sobrantes ni faltantes. Un muerto que habla o actúa deja además un uso o un estado, que siguen contando. Y una parada sin motivo no es gratis: le cuesta al autor una revisión y a la novela una reescritura que puede meter errores nuevos. Se descartó quitar el reparto del todo, porque las bases anteriores a RF3-PAS-09 perderían la detección. También se descartó cambiar la vista `presencia`: el conocimiento necesita la unión con el reparto, porque quien estaba en la escaleta y no se registró tampoco debe parar por conocimiento no adquirido.
