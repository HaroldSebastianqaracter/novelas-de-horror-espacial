...
| 5 | Solo se ofrecen las intenciones válidas para el estado y las acciones válidas para el tipo de parada | RF-FE-TAB-03, RF-FE-NOV-01, RF-FE-PAR-02 | Test parametrizado estado × acción y tipo × acción | `T` | Lo que valida el worker además del estado (puertas vigentes para `relanzar`); eso llega como rechazo | No: el rechazo del worker es la segunda barrera | `src/funcionalidades/ejecucion/acciones.test.ts` (estado × acción del tablero; tipo de parada × acción, pendiente del paso 5) | pendiente |
