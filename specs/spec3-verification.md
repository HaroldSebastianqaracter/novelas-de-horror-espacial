...
| 6 | El entrevistador no puede meter en el brief un dato que el comprador no dio | RF3-ENT-02 | Unit testing del filtro de citas literales | `T` | Una cita literal sacada de contexto («no tengo perro» → cita «perro») | Sí | `tests/test_entrevista.py::test_un_dato_sin_cita_literal_no_entra_en_el_brief` | implementado |
| 7 | El texto libre no cambia la configuración ni da instrucciones | RF3-ENT-05 | Red-teaming con inyección en el texto libre + unit testing de los campos permitidos | `T` | Inyecciones que no usan ningún patrón conocido: la búsqueda es una lista cerrada. El filtro de campos no depende de ella | No: filtro de campos + alerta | `tests/test_entrevista.py::test_el_texto_libre_no_cambia_la_configuracion_aunque_el_agente_obedezca` (con un agente que obedece la inyección) | implementado |
