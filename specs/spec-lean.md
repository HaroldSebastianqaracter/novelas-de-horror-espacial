...
- en la **generación normal**, se abre una parada de tipo `formal` en el primer capítulo con conflictos (en el último, si ningún conflicto trae capítulo: Lean ausente o un error de la herramienta), con el informe de Lean. Al relanzarla, cada capítulo con conflictos los recibe en su primer intento como los criterios incumplidos de la puerta 4. Salen del informe de esa parada, no del último resultado de la puerta 6 (el de un cambio fracasado se calculó sobre un canon que se deshizo), dejan de valer cuando la novela vuelve a completarse, y ni los fallos de la herramienta (`lean_no_disponible`, `lean_error`) ni un conflicto sin capítulo (el de un antecedente del mundo, que no escribe ningún capítulo) se le piden al redactor;
- en un **cambio del lector**, el cambio fracasa sin aplicar nada y devuelve el informe de Lean, sin parada.

De la base solo se lee la cronología, en la generación directamente y en el cambio del lector dentro de una simulación que se deshace; Lean compila después, **fuera de toda transacción**, para no tener el cerrojo de escritura mientras trabaja. Su resultado queda en `resultado_puerta` como puerta 6, también cuando el cambio del lector fracasa, y de ahí sale a Langfuse como score (`puerta_6` y un score por comprobación, RF3-OBS). La migración 014 admite `puerta = 6` en el `CHECK` de `resultado_puerta` y el tipo de parada `formal`, que se resuelve relanzando. La verificación la activa `config.cargar()` (se apaga con `NOVELAS_VERIFICACION_FORMAL=0`); un `Config` construido a mano, como el de los tests, la deja apagada para no compilar Lean en cada novela de prueba. Volver a completar una novela tras un cambio que fracasó no la repite: el canon es el ya verificado y no hay versión nueva.
