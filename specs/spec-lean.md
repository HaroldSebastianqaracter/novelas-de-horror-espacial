...
- en la **generación normal**, se abre una parada de tipo `formal` en el primer capítulo con conflictos (en el último, si ningún conflicto trae capítulo: Lean ausente o un error de la herramienta), con el informe de Lean. Al relanzarla, cada capítulo con conflictos los recibe en su primer intento como los criterios incumplidos de la puerta 4. Salen del informe de esa parada, no del último resultado de la puerta 6 (el de un cambio fracasado se calculó sobre un canon que se deshizo), dejan de valer cuando la novela vuelve a completarse, y los fallos de la herramienta (`lean_no_disponible`, `lean_error`) no se le piden al redactor;
