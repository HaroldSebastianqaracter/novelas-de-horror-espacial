...
**RF2-PIPE-13** *Amplía RF-PIPE-13.* El `resultado_puerta` de la puerta 4 lleva la parte mecánica **y** la de juicio en un solo registro: los conflictos de la mecánica y un conflicto por cada criterio que el juez da por `falla`, con su evidencia y su sugerencia. El veredicto es `falla` si falla cualquiera de las dos. Si la mecánica falla, el juez no se invoca y el registro lo dice. *Modificado por spec3, RF3-PAS-14 (24-09-2026): en la generación el juez corre siempre, y el redactor recibe la mecánica y el juicio juntos; solo la corrección del lector conserva el orden anterior.*
...
**RF2-PIPE-32** *Cambia RF2-PIPE-12 en el objeto sin traslado.* El objeto sin traslado es un **aviso**, no un conflicto: queda en el informe de la puerta 3 y no para el capítulo. Las reglas del mismo sitio (RF2-PIPE-26) y del poseedor (RF2-PIPE-28) siguen igual.

> **Decisión entrevistada, 24 de septiembre de 2026, tras la pasada real de tres capítulos.** El capítulo 2 paró porque la «Fibra dorsal», la línea que lleva calor y señal por toda la estación, aparecía en el anillo habitable con su última ubicación en la sala de antenas. La prosa no la traía al anillo: allí solo se hablaba de ella («Fibra para dos»). La escena la había puesto la escaleta, y el extractor, que sí sitúa un objeto cuando lo ve en un sitio nuevo, no la situó. Tal como está escrita, la comprobación solo salta en ese caso: si el extractor registra el objeto en la escena nueva, ese estado cuenta como traslado. Lo que detecta es que la escaleta y el canon no coinciden, no un error de la prosa. En las dos novelas reales saltó tres veces (la matriz madre de RF2-PIPE-26, la orden de misión de RF2-PIPE-28 y esta fibra), y las tres eran falsos positivos. **Revierte la decisión de RF2-PIPE-26**, que descartó rebajar la comprobación a aviso porque «perdía los traslados que sí faltan»: esos traslados solo los veía cuando el extractor callaba, y tres falsos positivos contra ninguna detección real inclinan la balanza. Se descartó distinguir las instalaciones fijas de los objetos portátiles (una marca en el elenco), que modela mejor el dominio pero toca el esquema, el elenco y su skill. **Lo que se pierde:** si la prosa lleva un objeto a otro sitio y el extractor no lo registra, la novela ya no para; queda como aviso en el informe (caso C05 del banco, cuyo recall baja del 50 % al 43 %). Y el revisor de continuidad deja de verlo, porque solo recibe lo que bloquea.
