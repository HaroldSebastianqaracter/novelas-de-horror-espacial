...
| Puerta 3: continuidad (contradicción factual, conocimiento no adquirido, presencia imposible, objeto sin traslado, coherencia temporal, día contra orden, entidad fuera de canon) | Programático | Tramo 2, dentro de la transacción que inserta texto y hechos | ROLLBACK y parada; la entidad fuera de canon y el objeto sin traslado solo avisan |
...
Desde spec3 la puerta 4 comprueba además los nombres exactos, los allegados, las etiquetas sin nombre, la longitud real del capítulo (aviso) y los términos vetados (guardrail, con su registro en `decision_politica`), y la puerta 5 avisa del elemento obligatorio que no aparece en ningún capítulo. La validación del capítulo y la política son los dos hooks del harness ([architecture.md](../architecture.md#los-dos-hooks-del-harness)). TLA+ se ejecuta en desarrollo. La validación visual con browser MCP está en [browser-mcp.md](browser-mcp.md); Lean 4 (puerta 6) y la rúbrica del LLM-as-judge ya están integrados. *Pendiente para la entrega*: la revisión humana. Todos envían su resultado a Langfuse como score.
