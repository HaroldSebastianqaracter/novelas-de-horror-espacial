...
| Presencia imposible | `Escena` (reparto), `Lugar` (`presenciaActual`), `EstadoObjeto`, `EstadoPersonaje` (`condicion`) | Un personaje en dos lugares a la vez; un muerto que reaparece; un objeto que aparece sin traslado registrado desde su última ubicación (esto último avisa y no para: spec2, RF2-PIPE-32). | Solo cubre entidades con ubicación registrada: un objeto que nunca se situó no puede estar mal situado. La muerte se lee de `condicion`, que **declara el extractor** (regla 3): una muerte que no registró como `muerto` no para nada, y por eso la acompaña una búsqueda del nombre del muerto en la prosa posterior. |
| Coherencia temporal | `LineaDeTiempo`, `Evento` (`ordenInterno`, `dia`) | Sucesos fuera de orden; un suceso posterior que cae en un día anterior (`dia_contra_orden`). | Verifica el orden declarado, no la plausibilidad de lo que cabe en una duración. **El día y el orden los declara el mismo extractor** (regla 3): compararlos entre sí detecta que se contradigan, no que los dos estén mal de la misma forma. Como el orden, no mira las analepsis ni los antecedentes del mundo, cuyo orden el extractor no conoce. Esos pares, y las edades que no cuadran, los comprueba Lean por el día (fila «Cronología formal»). |
| Cronología formal (Lean 4) | `Evento` (`dia`), la presencia en su escena, `Personaje` (`edad`, de donde sale el nacimiento), `EstadoPersonaje` (`condicion`) y los `Hecho` de edad | Alguien presente en un suceso antes de nacer o después de morir, también dentro de una analepsis y aunque no esté en el reparto planificado; una edad que la prosa afirma y no cuadra con el nacimiento; un antecedente del mundo después del día 0, la línea principal antes de él o fuera de orden, un recuerdo situado en el futuro de la historia. | Lean **demuestra** los invariantes, pero sobre los datos que le pasa el generador: días, presencias y muertes los declaró el extractor, así que ve que se contradigan entre sí, no que estén mal de la misma forma. La exportación de SQLite a Lean es Python sin prueba formal (la cubren un test de deriva y el ejemplo versionado). No ve eventos sin día, edades escritas en palabras, ubicuidad, partidas ni el orden dentro de un día (estar presente el día de la propia muerte, pero después, pasa). Y peca de estricto en un caso: un suceso dramatizado de una analepsis pone en él a todos los de la escena. Detalle en [spec-lean.md](../specs/spec-lean.md). |
...
> **Lean frente a los otros validadores, sobre novelas reales.** El enunciado pide un caso real en que Lean detecte lo que los demás no, o la razón de que no lo haya. **No lo hay, y la razón está en los datos, no en Lean.** Las tres novelas reales con capítulos escritos (la primera, de cuatro; la de tres y la de diez del 24 de septiembre, con 115 eventos, 300 presencias y 5 nacimientos) pasan Lean sin conflictos, pero tres de sus cuatro invariantes apenas tenían material: ninguna novela tiene una analepsis, solo una tiene una muerte y la prosa escribe las edades en letra, que el generador no exporta. El cuarto, el orden temporal, sí lo tenía (98 eventos de la línea principal y 11 antecedentes) y se cumplió. Lo que Lean ve y la puerta 3 no se demuestra con cinco casos construidos sobre la novela de demo, en los que la puerta 3 da verde y Lean para con el capítulo que hay que corregir (`tests/test_formal_lean.py::test_lean_ve_lo_que_la_puerta_3_no_ve`). La única incoherencia temporal real encontrada (una pérdida contada el día 2 en un capítulo y el «día tres» en otro) no la vio ningún validador, porque el extractor no le puso día al recuerdo. Detalle en el explainer [Verificación formal con Lean 4](proceso/explainers.md) y en la fila 8 de [spec-lean-verification.md](../specs/spec-lean-verification.md).

> **Decisión sin entrevistar, 24 de septiembre de 2026.** La cronología formal va en esta sección, con etiqueta `A`, aunque se ejecute como un proceso aparte: lo que la sostiene es una prueba (`decide` sobre proposiciones de Lean, más lemas generales que atan cada invariante a su lista de testigos), no la ejecución contra entradas de ejemplo. Sus invariantes se eligieron por lo que la puerta 3 **no** ve (analepsis, antecedentes del mundo, edades) y no por repetir lo que ya ve; la excepción es el orden de la línea principal, que repite `dia_contra_orden` a propósito para comparar los dos métodos sobre los mismos pares. Se descartó llevar a Lean la ubicuidad, que la puerta 3 ya comprueba con la contención de lugares, y las partidas, que la story bible no registra como dato cerrado. Las alternativas están en [trade-offs](proceso/trade-offs.md).
...
> **Decisión entrevistada, 24 de septiembre de 2026** (spec3, RF3-PAS-15). La novela de tres capítulos paró tres veces por una luz del pozo que cualquiera en la estación podía ver: el extractor marca lo observable, y la puerta 3 lo da por sabido a quien estuvo en ese lugar y avisa en los demás casos. Se descartaron pasar todo el conocimiento no adquirido a aviso, que dejaba de proteger los errores reales, y dejarlo como estaba, que con diez capítulos pararía varias veces por el mismo motivo.
...
**Las comprobaciones del cambio del lector** ([spec3, RF3-CAM-09](../specs/spec3.md)) son búsquedas dirigidas `T` sobre la corrección del revisor: que el nombre o el valor viejo ya no aparece (`cambio_sin_aplicar`), que el nombre de otra entidad que comparte una palabra con el viejo sigue ahí las mismas veces (`cambio_toca_otro`), que cada cita del revisor está en la prosa nueva y no estaba en la aprobada (`cambio_sin_cita`), que el capítulo apenas se movió (`cambio_desborda`, `difflib` por palabras) y que trae las mismas escenas (`cambio_escenas`). Sus puntos ciegos:
...
- **El nombre viejo llamado solo por una palabra que es, entera, el nombre de otra entidad** («Reyes» para referirse a «Nina Reyes» cuando existe otro «Reyes») es ambiguo y se deja como está: ni se sustituye ni cuenta como nombre viejo. Solo en ese caso: si la otra entidad solo comparte la palabra («Marta Ruiz» al renombrar «Tomás Ruiz»), «Ruiz» a secas sigue contando como nombre viejo. Se prefirió dejar una mención vieja ambigua a reescribir a la otra entidad, que es un fallo de canon seguro. `cambio_toca_otro` sostiene lo segundo aunque el revisor se equivoque (validador de 58e70f0); cuenta fuera del nombre viejo y del nuevo, para que el nuevo no tape un borrado (validador de 5717c7d).
- **La sustitución por código es literal**: un nombre viejo que la prosa escribe sin su tilde («Tomas» por «Tomás») no se sustituye en las fichas ni en los resúmenes que corrige el código. En la prosa sí lo caza `cambio_sin_aplicar`, que busca sin mirar tildes.
...
> **Decisión sin entrevistar, 23 de septiembre de 2026** (spec3, RF3-JUE-01). **Una opinión que no decide.** Cuando la puerta 3 para, un modelo lee cada conflicto y opina si parece real o un falso positivo. Es `I` y solo consultiva: la opinión se añade al informe de la parada, que ya está abierta, y ninguna rama del pipeline la lee para decidir. La continuidad sigue siendo `A`: la opinión le dice al autor dónde mirar, no si hay conflicto. Se descartó dejar que la opinión levantara paradas: el juez tiene recall y precisión limitados, y cambiaría una parada visible por un silencio.
