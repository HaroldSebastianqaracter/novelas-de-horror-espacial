...
| Coherencia temporal | `LineaDeTiempo`, `Evento` (`ordenInterno`, `dia`) | Sucesos fuera de orden; un suceso posterior que cae en un día anterior (`dia_contra_orden`). | Verifica el orden declarado, no la plausibilidad de lo que cabe en una duración. **El día y el orden los declara el mismo extractor** (regla 3): compararlos entre sí detecta que se contradigan, no que los dos estén mal de la misma forma. Las edades que no cuadran se comprueban con Lean (bloque 9 del plan), que lee `dia` y la edad de cada personaje. |
