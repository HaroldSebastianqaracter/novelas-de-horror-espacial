...
| **Juez de la novela** | `rubrica` | — (puntúa; `evaluacion_rubrica` la escribe el código) | 1, 2, 19, 25, 28 · LLM-as-judge con rúbrica de seis criterios, al terminar y sin bloquear |
...
| **Traza** | `ejecucion`, `llamada_modelo`, `resultado_puerta`, `traza_evento`, `langfuse_envio`, `decision_politica`, `evaluacion_rubrica` | Observabilidad: qué agente produjo qué y con qué contexto. `traza_evento` es lo que lee el stream. `langfuse_envio` apunta qué filas ya envió el worker a Langfuse (spec3, 3.4). `evaluacion_rubrica` guarda las notas del juez de la novela entera y las de la revisión humana, y sale a Langfuse como scores. `decision_politica` es el registro del guardrail: append-only y ninguna reversión la toca |
