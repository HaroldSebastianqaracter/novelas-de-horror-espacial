...
**Versión:** 1.12 — historial de cambios en `git log` sobre este archivo. Cambios v1.11 (Spec-X 02): `continuidad.filtrar_para_capitulo(log, entrada, personajes=None)` acepta el conjunto de personajes permitidos y el escritor le pasa todos (X-02.1); el esquema `Personaje` (§4) gana el campo `posicion: str = ""`, que fija el extractor (X-02.2); `resumen_rodante.agregar(texto, n, resumen)` deja de recortar por ventana —guarda todos los capítulos— y `resumen_rodante.para_escritor(texto, ventana)` entrega los últimos completos y los anteriores en una línea (X-02.3); el prompt del escritor gana el marcador `{{RELEVO}}` con la locación del capítulo siguiente (X-02.4b). Cambios v1.12: el corte de QA (§6) se solapa con la escritura del capítulo siguiente, porque audita capítulos ya cerrados y no depende del que viene; el corte se cierra **antes** que ese capítulo, y `aplicar_delta` lo exige, porque `ultimo_capitulo_cerrado` es de donde los hooks sacan el capítulo de QA (§13.3) y moverlo a media auditoría rompería su `validar-reporte` contra H-11.
...
**Forma canónica del comando.** A lo largo de este documento los comandos se escriben `python -m app <verbo>` por brevedad, pero el intérprete real es siempre el del entorno virtual del proyecto, como fija `CLAUDE.md`. H-11 acepta **una sola forma**, la del entorno virtual, con el verbo y el número exactos y nada más. No acepta `python` a secas, y el motivo es justo el inverso del que parece: en esta máquina `python` **sí** resuelve, a una instalación del sistema distinta del entorno virtual del proyecto. Un validador ejecutado bajo otro intérprete puede dar otro veredicto —le faltan las dependencias, o peor, las tiene en otra versión— y una validación que pasa por el motivo equivocado es más dañina que una que falla. El harness tiene que saber con qué intérprete se validó cada artefacto, y la única forma de saberlo es no admitir más que uno. Una forma, un comando, un número.
...
- El frontend lanza con `--permission-prompts none`. Verificado contra el `--help` de la CLI instalada: con `--print`, ese valor hace que lo que se preguntaría se **deniegue automáticamente** en vez de quedarse esperando una respuesta que no va a llegar. Es lo que se quiere: una fase que muere con una denegación anotada es depurable; una que se cuelga consume la sesión entera sin decir por qué.
- La sintaxis de las entradas de `permissions.allow` que autorizan a los subagentes **sigue sin verificar** y hay que leerla de la versión instalada antes de escribirlas. Una regla mal escrita no falla: simplemente no autoriza, y la fase muere en la primera acción que necesitaba ese permiso.
