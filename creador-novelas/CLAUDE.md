...
- `04_estado/`: style_guide, tres_actos, capitulos.json, personajes.json, mundo.json, continuidad.json (append-only), resumen_rodante.md, recursos_narrativos.json (RF-05.5: recursos de prosa acumulados por `aplicar-delta` desde el campo `recursos_narrativos` del delta), manifest.json. Solo los escribe el CLI. `prompts/` y `deltas/` son artefactos de trabajo: `deltas/delta_cap_N.json` lo escribe y valida el extractor.
...
- `07_registro/tanda_<ts>/`: registro de ejecución (RF-08.5, §5.1): `eventos.jsonl`, `prompts/`, `retornos/`, `descartados/`, `uso.jsonl`, `tanda.json` (prompts_hash y dimensionamiento de esa tanda), `cursor.json` final. Lo escriben el CLI y los hooks; el orquestador no lo lee ni lo escribe (H-09): `status` lo resume. Se publica en Langfuse **después** de la tanda con `python -m app exportar-traza <tanda|ultima> [--con-cuerpos] [--volcar archivo] [--solo-volcar]` (RF-09, §16): credenciales solo por entorno (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_HOST`), por defecto sin prosa ni prompts, reexportar no duplica.
...
- Antirrepetición (RF-05.5, §17), sin tocar INV-01: `validar-capitulo` rechaza toda secuencia de 4+ palabras con contenido léxico repetida literalmente de un capítulo anterior (lo lee el código, no el agente; nombres del registro y palabras vacías no cuentan) y nombra la frase y el capítulo de origen; el extractor reporta `recursos_narrativos` en su delta y `preparar-capitulo` inyecta los más usados al escritor como recursos **agotados**, no prohibidos.
