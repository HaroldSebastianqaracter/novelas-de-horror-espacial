...
const SALUDO_MS = 2 * 4 * 180; // dos ciclos de «saludar»
const SUENO_MS = 60_000;
const CLAVE_OCULTO = "astronauta-oculto";
...
// Sin matchMedia (jsdom) se trata como «reducir movimiento»: la imagen quieta, sin bucle de animación.
 ...
 * ratón, así que no tapa controles. Al pulsarlo se esconde, y se recuerda en este navegador.
 */
export function Astronauta({ modo = "pasear", className }: { modo?: Modo; className?: string }) {
  const [oculto, setOculto] = useState(leerOculto);
  const [quieto] = useState(reducirMovimiento);
  ...
  const esconder = () => {
    ...
    try {
      ...
      // Sin almacenamiento, se esconde solo hasta recargar.
    }
  };
